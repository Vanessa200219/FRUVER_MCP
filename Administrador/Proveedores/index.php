...
if (!isset($_SESSION['NumerodeDocumento'])) {
	...
	header('Location: ../../Cuenta/Vista/iniciarsesion.php');
		...
		$sentencia = $bd->query("SELECT * FROM persona INNER JOIN roles ON persona.id_rol = roles.Id WHERE id_rol = 4;");
...
<div class="div__firmts">
    <a href="#form" class="empleado">Nuevo Proveedor</a>
		...
		<section class="buscador">
			<div >
				<form class="buscar" action="" method="get">
					...
					<input id="Buscar" type="text" name="Busqueda" placeholder="Buscar Proveedor" required>
			...
			<?php
				...
				if (isset($_GET['Buscar'])) {
					...
					$sentencia = $bd->query("SELECT * FROM persona INNER JOIN roles ON persona.id_rol = roles.Id WHERE NumerodeDocumento LIKE '%$busqueda%' AND id_rol = 4;");
					$sentencia->execute();
					...
					<table class="table__1">
						...
						<tr class="content">
							...
							<td><a class="editar" href="Vista/informacion.php?id=<?php echo $dato['NumerodeDocumento']; ?>">Ver más</a></td>
						</tr>
